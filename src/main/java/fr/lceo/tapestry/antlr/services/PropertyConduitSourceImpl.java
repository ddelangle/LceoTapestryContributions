 ...
 * <b>Fichier</b> : PropertyConduitSourceImpl.java
 * <b>Créé le</b> : 24 août 2010
 *
 */
package fr.lceo.tapestry.antlr.services;

import static fr.lceo.tapestry.antlr.parser.PropertyExpressionParserParser.DECIMAL;
import static fr.lceo.tapestry.antlr.parser.PropertyExpressionParserParser.DEREF;
import static fr.lceo.tapestry.antlr.parser.PropertyExpressionParserParser.FALSE;
import static fr.lceo.tapestry.antlr.parser.PropertyExpressionParserParser.IDENTIFIER;
import static fr.lceo.tapestry.antlr.parser.PropertyExpressionParserParser.INTEGER;
import static fr.lceo.tapestry.antlr.parser.PropertyExpressionParserParser.INVOKE;
import static fr.lceo.tapestry.antlr.parser.PropertyExpressionParserParser.LIST;
import static fr.lceo.tapestry.antlr.parser.PropertyExpressionParserParser.NOT;
import static fr.lceo.tapestry.antlr.parser.PropertyExpressionParserParser.NULL;
import static fr.lceo.tapestry.antlr.parser.PropertyExpressionParserParser.RANGEOP;
import static fr.lceo.tapestry.antlr.parser.PropertyExpressionParserParser.SAFEDEREF;
import static fr.lceo.tapestry.antlr.parser.PropertyExpressionParserParser.STRING;
import static fr.lceo.tapestry.antlr.parser.PropertyExpressionParserParser.THIS;
import static fr.lceo.tapestry.antlr.parser.PropertyExpressionParserParser.TRUE;
...
import fr.lceo.tapestry.antlr.parser.PropertyExpressionLexerLexer;
import fr.lceo.tapestry.antlr.parser.PropertyExpressionParserParser;

/**
 * Implementation of {@link PropertyConduitSource} using antlr 3.0.0
 * 
 * It is based on {@link org.apache.tapestry5.internal.services.PropertyConduitSourceImpl} revision 986533
 * 
 * No modification were done on code except the use of specific parser and lexer.
 * 
 * @author denisdelangle
 * 
 */
public class PropertyConduitSourceImpl implements PropertyConduitSource, InvalidationListener {
	...
	private static final MethodSignature SET_SIGNATURE = new MethodSignature(void.class, "set", new Class[] { Object.class, Object.class }, null);
	...
	/**
	 * Because of stuff like Hibernate, we sometimes start with a subclass in some inaccessible class loader and need to work up to a base class from a common
	 * class loader.
	 */
	...
	class PropertyConduitBuilder {
		...
		private void createGetRoot() {
			...
			builder.addln("if (root == null) throw new NullPointerException(\"Root object of property expression '%s' is null.\");", expression);
		...
		private void createAccessors() {
			...
			MethodSignature sig = new MethodSignature(activeClass, "navigate", new Class[] { rootType }, null);
		 ...
		 * @param rootName
		 *            name of variable holding reference to root object of expression
		...
		private GeneratedTerm subexpression(BodyBuilder builder, Tree node, String rootName) {
			...
			while (node != null) {
				...
				case SAFEDEREF:
					...
					GeneratedTerm generated = processDerefNode(builder, activeType, node, previousReference, rootName);
				...
				case INVOKE:
					...
					generated = addAccessForMember(builder, activeType, node, previousReference, rootName, NullHandling.IGNORE);
				...
				default:
					throw unexpectedNodeType(node, TRUE, FALSE, INTEGER, DECIMAL, STRING, DEREF, SAFEDEREF, IDENTIFIER, INVOKE, LIST);
				}
			}
		...
		private void createNoOpSetter() {
			createNoOp(classFab, SET_SIGNATURE, "Expression '%s' for class %s is read-only.", expression, rootType.getName());
		}
		...
		private void createGetter(MethodSignature navigateMethod, Tree node, ExpressionTermInfo info) {
			...
			if (method == null && !info.isField()) {
				createNoOp(classFab, GET_SIGNATURE, "Expression %s for class %s is write-only.", expression, rootType.getName());
				return;
			}
			...
			String reference = info.isField() ? info.getPropertyName() : createMethodInvocation(builder, node, "root", method);
		 ...
		 * 
		 * @param bodyBuilder
		 *            may receive new code to define variables for some sub-expressions
		 * @param node
		 *            the INVOKE node; child #1 and up are parameter expressions to the method being invoked
		 * @param rootName
		 *            name of variable holding reference to root object of expression
		 * @param method
		 *            defines the name and parameter types of the method to invoke
		 * @return method invocation string (the name of the method and any parameters, ready to be added to a method body)
		 */
		 ...
		 *            name of variable holding reference to root object of expression
		 * @param childOffset
		 *            the offset to the first parameter (for example, this is 1 for an INVOKE node)
		 * @param method
		 *            defines the name and parameter types of the method to invoke
		 * @return method invocation string (the name of the method and any parameters, ready to be added to a method body)
		 */
		private String createMethodInvocation(BodyBuilder bodyBuilder, Tree node, String rootName, int childOffset, Method method) {
			...
			for (int i = 0; i < parameterTypes.length; i++) {
				...
				if (!parameterType.isAssignableFrom(actualType)) {
					...
					String call = String.format("coerce(($w) %s, %s)", currentReference, addInjection(Class.class, parameterType));

					String parameterTypeName = ClassFabUtils.toJavaClassName(parameterType);

					bodyBuilder.addln("%s %s = %s;", parameterTypeName, coerced, ClassFabUtils.castReference(call, parameterTypeName));
		 ...
		 */
		private GeneratedTerm processDerefNode(BodyBuilder builder, Type activeType, Tree node, String previousVariableName, String rootName) {
			...
			return addAccessForMember(builder, activeType, term, previousVariableName, rootName, allowNull ? NullHandling.ALLOW : NullHandling.FORBID);
		}
		...
		private GeneratedTerm addAccessForMember(BodyBuilder builder, Type activeType, Tree term, String previousVariableName, String rootName,
			...
			if (method == null && !info.isField())
				throw new RuntimeException(String.format("Property '%s' of class %s is not readable (it has no read accessor method).", info.getDescription(),
						activeClass.getName()));

			Type termType;
			/*
			 * It's not possible for the ClassPropertyAdapter to know about the generic info for all the properties of a class. For instance; if the type
			 * arguments of a field are provided by a subclass.
			 */
			if (info.isField()) {
			...
			String reference = info.isField() ? info.getPropertyName() : createMethodInvocation(builder, term, rootName, method);
			...
			case FORBID:
				...
				builder.addln("if (%s == null) %s.nullTerm(\"%s\", \"%s\", $1);", variableName, PropertyConduitSourceImpl.class.getName(),
						info.getDescription(), expression);
				break;
		...
		private RuntimeException unexpectedNodeType(Tree node, int... expected) {
			...
			for (int i = 0; i < expected.length; i++)
				tokenNames.add(PropertyExpressionParserParser.tokenNames[expected[i]]);

			String message = String.format("Node %s was type %s, but was expected to be (one of) %s.", node.toStringTree(),
					PropertyExpressionParserParser.tokenNames[node.getType()], InternalUtils.joinSorted(tokenNames));
		...
		private ExpressionTermInfo infoForPropertyOrPublicField(Class activeType, Tree node) {
			...
			if (adapter == null) {
				...
				throw new UnknownValueException(String.format("Class %s does not contain a property (or public field) named '%s'.", activeType.getName(),
		...
		private ExpressionTermInfo infoForInvokeNode(Class activeType, Tree node) {
			...
			try {
				...
				if (method.getReturnType().equals(void.class))
					throw new RuntimeException(String.format("Method %s.%s() returns void.", activeType.getName(), methodName));
				...
				throw new RuntimeException(String.format("No public method '%s()' in class %s.", methodName, activeType.getName()));
			}
		}
		...
		private Method findMethod(Class activeType, String methodName, int parameterCount) throws NoSuchMethodException {
			for (Method method : activeType.getMethods()) {
				...
				if (method.getParameterTypes().length == parameterCount && method.getName().equalsIgnoreCase(methodName))
					return method;
			}
	...
	public PropertyConduitSourceImpl(PropertyAccess access, @ComponentLayer ClassFactory classFactory, TypeCoercer typeCoercer, StringInterner interner) {

		System.out.println("PropertyConduitSourceImpl Override used");
		this.access = access;
	...
	/**
	 * Clears its caches when the component class loader is invalidated; this is because it will be common to generate conduits rooted in a component class
	 * (which will no longer be valid and must be released to the garbage collector).
	 */
	...
	/**
	 * Builds a subclass of {@link BasePropertyConduit} that implements the get() and set() methods and overrides the constructor. In a worst-case race
	 * condition, we may build two (or more) conduits for the same rootClass/expression, and it will get sorted out when the conduit is stored into the cache.
	 * 
	 * @param rootClass
	...
	private PropertyConduit build(final Class rootClass, String expression) {
		...
		try {
			...
			throw new PropertyExpressionException(
					String.format("Exception generating conduit for expression '%s': %s", expression, InternalUtils.toMessage(ex)), expression, ex);
		}
	}
	...
	private <T> PropertyConduit createLiteralConduit(Class<T> type, T value) {
		return new LiteralPropertyConduit(type, invariantAnnotationProvider, interner.format("LiteralPropertyConduit[%s]", value), typeCoercer, value);
	}
	...
	private Tree parse(String expression) {
		...
		PropertyExpressionLexerLexer lexer = new PropertyExpressionLexerLexer(ais);

		CommonTokenStream tokens = new CommonTokenStream(lexer);

		PropertyExpressionParserParser parser = new PropertyExpressionParserParser(tokens);
